...
int main(int argc, char **argv){
  ...
  /*Run this once to malloc all the input, output, and temporary variable arrays */

  //fscanf(fp, "%s", gate);
  //if(strcmp(gate, "INPUTVAR")){//input gate
  /* Input comes first always */
  fscanf(fp, "%s ", name);//take in inputVar
  ...
  /* Output comes second always */
    /*}else if(strcmp(gate, "OUTPUTVAR")){//output gate */
  fscanf(fp, "%s", name);
  ...
  for(unsigned int i = 0; i < pow(2, input_size); i++){//loops base on the number of grey code
    ...
    // for(unsigned int i = 0; i < pow(2, input_grey_code->num_of_bits) ;i++){
    //   printf("%d ", input_grey_code->grey_code_sequence[i]);
    // }
    ...
    /* Print Our storage Arrays */
    /* Searching within the Inputs */
    // for(unsigned int i = 0; i < input_size; i++){
    //     printf("%s %d ", input_variable[i].name, input_variable[i].value);
    // }
    // /* Searching within the Outputs */
    // for(unsigned int i = 0; i < output_size; i++){
    //   printf("%s %d ", output_variable[i].name, output_variable[i].value);
    // }
    ...
    while(fscanf(fp, "%s", gate) != EOF){
        if(strcmp(gate, "NOT") == 0){
            ...
            for(unsigned int i = 0; i < multiplexer_input_size; i++){
              fscanf(fp, "%s", input1);
              input_arr[i] = getValue(input1);
            }
            /* Selector */
            temp = 0;
            for(unsigned int i = 0; i < selector_size; i++){
              fscanf(fp, "%s", input1);//the input value
                temp += getValue(input1) * pow(2, selector_size-1-i);
            }
            /* Output */
            fscanf(fp, "%s", output);
            for(unsigned int i = 0; i < multiplexer_input_size; i++){
